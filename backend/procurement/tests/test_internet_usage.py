...
class InternetUsageDashboardTests(TestCase):
    ...
    def test_reports_existing_real_response_bytes_without_hot_path_instrumentation(self):
        ...
        self.assertEqual(response.data["periods"]["24h"]["extraction_download_bytes"], 1536)
        self.assertEqual(response.data["recent_runs"][0]["download_bytes"], 1536)
        ...
        self.assertFalse(response.data["categories"][2]["measured"])
