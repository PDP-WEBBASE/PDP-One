...
test("obsolete Rancher artifacts are pruned while manifest-protected active, previous, and in-flight images and all volumes are retained", async () => {
  const guard = await read("scripts/windows/Invoke-PDPOneDiskGuard.ps1");
  const compose = await read("docker-compose.yml");

  assert.match(guard, /PDPOne\.ReleaseManifest\.ps1/);
  ...
  assert.match(guard, /state\\release-manifests\\/);
