...
test('normal application activation excludes Nginx and Tailscale while preserving force-recreate', () => {
  assert.match(
    deployment,
  ...
  assert.match(
    deployment,
    /if \(\$scope\.topology_sensitive\) \{\s*Invoke-PDPOneNative[^\n]+\(@\("compose", "up", "--detach", "--no-build", "--pull", "never", "--no-deps", "--force-recreate"\) \+ \$applicationTopologyTargets\)/s,
  );
  assert.match(
    deployment,
    /@\("compose", "up", "--detach", "--no-build", "--pull", "never", "--no-deps", "--force-recreate"\) \+ \$activationTargets/,
  );
  assert.doesNotMatch(
    deployment,
...
test('accepted exact Web build verification remains enabled without restarting the Edge', () => {
  assert.match(deployment, /if \("web" -in \$changedServices\)/);
  assert.match(deployment, /-ExpectedWebBuildId/);
  assert.match(deployment, /"content-" \+ \[string\]\$fingerprints\.web/);
});
...
test('deployment state records whether the Edge was impacted, refreshed or preserved', () => {
  for (const field of [
