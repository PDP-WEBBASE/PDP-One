...
function syncMetadataColumn(article: HTMLElement) {
  ...
  if (badgeGroup) {
    ...
    badgeGroup.querySelectorAll<HTMLElement>(".pdp-preview-classification-badge").forEach((badge) => sourceRow.appendChild(cloneVisual(badge)));
  }
...
function BulkBar({
  ...
  return <div className="pdp-v2-bulk-bar" dir="ltr">
    <label className="pdp-v2-select-all" dir="rtl" title="انتخاب همه این صفحه" aria-label="انتخاب همه این صفحه"><input type="checkbox" checked={allSelected} disabled={!pageIds.length || busy} onChange={(event) => onTogglePage(event.target.checked)} /></label>
...
export default function ProcurementCardLayoutBulkRemoveV2() {
  ...
  return <>
    <style>{`
      ...
      #${BULK_HOST_ID}{min-width:0;align-self:end}.pdp-v2-bulk-bar{display:flex;align-items:center;justify-content:flex-start;gap:6px;min-height:32px;flex-wrap:nowrap}.pdp-v2-select-all{display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;padding:0;border:1px solid #cbd5e1;border-radius:9px;background:#fff;color:#334155}.pdp-v2-select-all input{width:15px;height:15px;margin:0;accent-color:#145563}.pdp-v2-selected-count{display:inline-flex;align-items:center;min-height:32px;padding:4px 7px;border:1px solid #e2e8f0;border-radius:9px;background:#f8fafc;color:#64748b;font-size:10px;white-space:nowrap}.pdp-v2-bulk-remove{height:32px;padding:4px 9px;border:1px solid #fecaca;border-radius:9px;background:#fff1f2;color:#be123c;font:inherit;font-size:10.5px;font-weight:700;white-space:nowrap;cursor:pointer}.pdp-v2-bulk-remove:disabled{opacity:.45;cursor:not-allowed}
