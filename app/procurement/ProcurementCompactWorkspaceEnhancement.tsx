...
function activeButtonLabel(labels: Set<string>, excludeNav = false) {
...
function activeTopTab() {
  return activeButtonLabel(TOP_LABELS);
}

function activeWorkflow() {
  return activeButtonLabel(WORKFLOW_LABELS, true);
}
...
function fieldValue(labelPrefix: string) {
  ...
  const workflowButton = Array.from(document.querySelectorAll<HTMLButtonElement>("button")).find((button) =>
    normalize(button.textContent) === workflow && Boolean(normalize(button.className)),
  );
  const root = workflowButton?.closest("section") || document.body;
  ...
  return normalize(field?.value) || (top ? "" : "");
}
...
function installCompactFetchGuard() {
  ...
  const nativeFetch = guarded.__pdpPaginationNativeFetch.bind(window);
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    const response = await nativeFetch(`${COMPACT_NOTICE_PATH}?${params.toString()}`, init);
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent(PAGINATION_META_EVENT, { detail: {
        ...
        pageSize: Number(payload.page_size || 50),
        contextKey: JSON.stringify([top, workflowCode(activeWorkflow(), forceRecommended), ...Array.from(params.entries())]),
      } }));
...
function findWorkflowSection() {
  ...
  const button = Array.from(document.querySelectorAll<HTMLButtonElement>("button")).find((candidate) =>
    normalize(candidate.textContent) === workflow && Boolean(normalize(candidate.className)),
  );
...
function enhanceRecordCards(payload: NoticePayload | null) {
  ...
  if ((top === "مناقصات" || top === "استعلامات") && payload) {
    records.slice(0, payload.results.length).forEach((article, index) => {
      ...
      cluster.querySelectorAll<HTMLElement>("[data-pdp-compact-badges]").forEach((node) => node.remove());
      ...
      cluster.querySelectorAll<HTMLAnchorElement>("a").forEach((anchor) => {
        if (sourceNames.has(normalize(anchor.textContent))) anchor.style.display = "none";
      });
      ...
      if (facts) {
        ...
        if (!visible) facts.style.display = "none";
      }
    });
  }
  ...
  records.forEach((article) => {
    ...
    if (dismissHost) dismissHost.style.marginTop = "0";
...
export default function ProcurementCompactWorkspaceEnhancement() {
  ...
  useEffect(() => {
    const syncDom = () => {
      ...
      const top = activeTopTab();
      if (previousTop.current && previousTop.current !== top) {
        setDeadlineStatus("");
        setPublishedOn("");
        (window as CompactWindow).__pdpPaginationPage = 1;
      }
      ...
      if (top === "داشبورد مدیریتی" && !dashboard) refreshDashboard();
    };
    syncDom();
    const observer = new MutationObserver(syncDom);
    observer.observe(document.body, { childList: true, subtree: true });
    const timer = window.setInterval(syncDom, 700);
    return () => { observer.disconnect(); window.clearInterval(timer); };
  }, [noticePayload, sourceNames, dashboard, refreshDashboard]);

  useEffect(() => {
    const handleSync = (event: Event) => {
      const detail = (event as CustomEvent<ProcurementUiSyncDetail>).detail;
      if (detail?.source === "compact-workspace") return;
      if (detail?.dashboard || detail?.bulkWorkspace || activeTopTab() === "داشبورد مدیریتی") refreshDashboard();
    };
  ...
  const showBulk = activeTopTab() && (activeTopTab() === "مناقصات" || activeTopTab() === "استعلامات") && activeWorkflow() === "پیشنهادی";
