...
const compactRecordStyle = { padding:14, gap:14 } as const;
const compactDecisionStyle = { gap:7, paddingInlineStart:14 } as const;

export default function ProcurementWorkspaceV11() {
  ...
  return <main className={styles.page} dir="rtl">
    ...
    {tab === "direct" && <section>
      ...
      <div className={styles.recordList}>{filteredDirect.map((item,index)=>{const u=urgency(item.targetDeadline);return <article className={styles.record} style={compactRecordStyle} key={item.id}><div><div className={styles.recordTop}><small><b>ردیف {fa.format(index+1)}</b>{item.referenceCode&&directView!=="all"&&directView!=="recommended"&&<> · <span className={styles.codeBadge}>{item.referenceCode}</span></>} · {item.opportunityType}</small><div style={{display:"flex",gap:6,alignItems:"center",marginInlineStart:"auto",flexWrap:"wrap"}}><span className={`${styles.urgency} ${styles[u.tone]}`}>{u.label}</span><button className={styles.secondaryButton} style={compactViewStyle}>مشاهده</button></div></div><h3 style={{margin:"6px 0 4px"}}>{item.title}</h3><p>{item.employer}</p><div className={styles.facts} style={{marginTop:8}}><span>{item.domain}</span><span>{item.province}</span><span>اهمیت: {importanceLabels[item.importance]}</span>{item.probability!==null&&<span>احتمال تبدیل: {fa.format(item.probability)}٪</span>}</div></div><div className={styles.decision} style={compactDecisionStyle}><span className={styles.stage}>{item.stage === "new" ? "کل ارجاعات مستقیم" : item.stage === "reviewing" ? "پیشنهادی" : item.stage === "submitted" ? "ارسال‌شده" : "منتخب"}</span><dl style={{margin:0}}><div style={{padding:"3px 0"}}><dt>مسئول</dt><dd>{item.responsible || "تعیین نشده"}</dd></div></dl></div></article>})}</div>
    </section>}
