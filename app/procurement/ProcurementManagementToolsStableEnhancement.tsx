...
type ToolKey = "workflow" | "review" | "followup" | "contract" | "dashboard" | "automation" | "analysis" | "analysisSettings" | "engine";
...
const LEGACY_FLOATING_LABELS = new Set([
  ...
  "داشبورد مدیریتی", "زمان‌بندی استخراج و AI", "مرکز تحلیل فراخوان‌ها", "تنظیمات تحلیل واقعی", "موتور تحلیل PDP",
]);
...
function ManagementToolbar({ onOpen }: { onOpen: (key: ToolKey) => void }) {
  return <section dir="rtl" style={{margin:"12px 0 16px",padding:14,border:"1px solid #dbe3ec",borderRadius:14,background:"#f8fafc"}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:10,marginBottom:10,flexWrap:"wrap"}}><div><strong style={{display:"block",fontSize:17}}>ابزارهای مدیریتی زیرسامانه</strong><small style={{color:"#64748b"}}>ابزارهای مدیریتی و تحلیل بدون پایش دائمی DOM در این بخش قرار دارند.</small></div><span>۹ ابزار</span></div>
