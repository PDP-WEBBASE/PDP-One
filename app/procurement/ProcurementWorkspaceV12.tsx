...
const importanceStyles: Record<Importance, React.CSSProperties> = {
...
export default function ProcurementWorkspaceV12() {
  ...
  }), [tab, noticeView, search, sourceFilter, provinceFilter, importanceFilter, urgencyFilter]);

  const filteredDirect = useMemo(() => directReferrals.filter((item) => {
    const currentUrgency = urgency(item.targetDeadline);
  ...
  }), [directView, search, provinceFilter, importanceFilter, urgencyFilter, directTypeFilter]);
  ...
  return <main className={styles.page} dir="rtl">
    ...
    {tab === "dashboard" && <section>
      <div className={styles.kpis}>
        <article className={styles.kpi}><span>فراخوان جدید</span><b style={{fontSize:21,lineHeight:1.15}}>{fa.format(24)}</b><small>از آخرین استخراج</small></article>
    ...
    {(tab === "tenders" || tab === "inquiries") && <section>
      ...
      <div className={styles.recordList}>{filteredNotices.map((item,index) => { const u=urgency(item.deadline); return <article className={styles.record} style={compactRecordStyle} key={item.id}>
        <div>
          <div className={styles.recordTop}>
            ...
            <div style={{display:"flex",gap:5,alignItems:"center",marginInlineStart:"auto",flexWrap:"wrap"}}>
              <a href={item.sourceUrl} target="_blank" rel="noreferrer" style={sourceBadgeStyle}>{item.source}</a>
              <span style={{...importanceBadgeBase,...importanceStyles[item.importance]}}>اهمیت {importanceLabels[item.importance]}</span>
              <span className={`${styles.urgency} ${styles[u.tone]}`}>{u.label}</span>
              <button className={styles.secondaryButton} style={compactViewStyle}>مشاهده</button>
            </div>
          </div>
          <h3 style={{margin:"4px 0 2px",fontSize:17}}>{item.title}</h3><p>{item.employer}</p>
          <div className={styles.facts} style={{marginTop:5,gap:5}}><span>{item.province}</span><span>{u.remaining}</span><span>اولویت: {item.score ?? "تحلیل نشده"}</span>{item.documents > 0 && <span>{fa.format(item.documents)} سند</span>}</div>
        </div>
        <div className={styles.decision} style={compactDecisionStyle}><span className={styles.stage}>{item.result || item.stage || (item.recommended ? "پیشنهادی" : allLabel(tab))}</span><dl style={{margin:0}}><div style={{padding:"2px 0"}}><dt>مسئول</dt><dd>{item.responsible || "تعیین نشده"}</dd></div></dl>{!item.stage && <div className={styles.actions}><button className={styles.primaryButton} style={{padding:"6px 9px"}}>افزودن به پیشنهادی</button></div>}</div>
      </article>; })}</div>
    </section>}
    ...
    {tab === "direct" && <section>
      ...
      <div className={styles.recordList}>{filteredDirect.map((item,index) => { const u=urgency(item.targetDeadline); return <article className={styles.record} style={compactRecordStyle} key={item.id}>
        <div><div className={styles.recordTop}><small><b>ردیف {fa.format(index+1)}</b>{item.referenceCode && directView !== "all" && directView !== "recommended" && <> · <span className={styles.codeBadge}>{item.referenceCode}</span></>} · {item.opportunityType}</small><div style={{display:"flex",gap:5,alignItems:"center",marginInlineStart:"auto",flexWrap:"wrap"}}><span style={{...importanceBadgeBase,...importanceStyles[item.importance]}}>اهمیت {importanceLabels[item.importance]}</span><span className={`${styles.urgency} ${styles[u.tone]}`}>{u.label}</span><button className={styles.secondaryButton} style={compactViewStyle}>مشاهده</button></div></div><h3 style={{margin:"4px 0 2px",fontSize:17}}>{item.title}</h3><p>{item.employer}</p><div className={styles.facts} style={{marginTop:5,gap:5}}><span>{item.domain}</span><span>{item.province}</span>{item.probability !== null && <span>احتمال تبدیل: {fa.format(item.probability)}٪</span>}</div></div>
        <div className={styles.decision} style={compactDecisionStyle}><span className={styles.stage}>{item.stage === "new" ? "کل ارجاعات مستقیم" : item.stage === "reviewing" ? "پیشنهادی" : item.stage === "submitted" ? "ارسال‌شده" : "منتخب"}</span><dl style={{margin:0}}><div style={{padding:"2px 0"}}><dt>مسئول</dt><dd>{item.responsible || "تعیین نشده"}</dd></div></dl></div>
      </article>; })}</div>
    </section>}
