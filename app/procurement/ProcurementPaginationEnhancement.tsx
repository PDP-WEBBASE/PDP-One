...
function activeWorkflow() {
  return activeButtonLabel(WORKFLOW_LABELS);
}
...
function findListSection() {
  ...
  const workflowButton = Array.from(document.querySelectorAll<HTMLButtonElement>("button")).find((button) =>
    WORKFLOW_LABELS.has(normalize(button.textContent)) && Boolean(normalize(button.className)),
  );
...
export default function ProcurementPaginationEnhancement() {
  installPaginationFetchGuard();
  const [host, setHost] = useState<HTMLElement | null>(null);
  const [meta, setMeta] = useState<PaginationMeta>({ count: 0, page: 1, pageSize: 50, contextKey: "" });

  useEffect(() => {
    let searchTimer = 0;
    let syncTimer = 0;
    const refreshHost = () => setHost(ensureHost());
    ...
    const onClick = (event: MouseEvent) => {
      ...
      const label = normalize(button?.textContent);
      if (TOP_LABELS.has(label) || WORKFLOW_LABELS.has(label) || label === "پاک‌کردن") resetAndRefresh(0);
    };
    ...
    const onChange = (event: Event) => {
      ...
      if (["منبع", "استان", "اهمیت", "فوریت", "نوع ارجاع"].some((prefix) => text.startsWith(prefix))) resetAndRefresh(0);
    };
  ...
  return host ? createPortal(<PaginationBar meta={meta} />, host) : null;
}
