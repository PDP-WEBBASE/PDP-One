...
$InstallerVersion = "2026.07.19.17"
...
function Test-PostgresDataVolume([string]$VolumeName, [string]$ImageName) {
    ...
    try {
        ...
        & docker run --rm `
            --volume "${VolumeName}:/var/lib/postgresql/data:ro" `
            --entrypoint sh `
            $ImageName `
            -c "test -s /var/lib/postgresql/data/PG_VERSION" *> $null
...
if ($adminSyncCode -ne 0) { throw "The trial administrator account could not be synchronized." }

$adminUser = ((Select-String -LiteralPath $EnvPath -Pattern '^PDP_TRIAL_ADMIN_USERNAME=(.*)
...
Start-Process -FilePath (Join-Path $ProjectRoot "CONNECT-CHATGPT.bat") -WorkingDirectory $ProjectRoot
    ...
    if ($startCode -ne 0) { throw "The existing PostgreSQL service could not be started for recovery." }
    ...
    if (-not $databaseReady) { throw "The existing PostgreSQL data did not become ready for recovery." }
    ...
    try {
        ...
        & docker compose exec -T --user postgres db `
            psql --username $PostgresUser --dbname $PostgresDb `
            --set "ON_ERROR_STOP=1" --command $sql 2>&1 | Out-Host
        $repairCode = $LASTEXITCODE
    } finally {
        $ErrorActionPreference = $previousErrorAction
    }
    if ($repairCode -ne 0) {
        throw "The existing PostgreSQL password could not be synchronized. The data volume was not modified or deleted."
    }
...
$adminPassword = ((Select-String -LiteralPath $EnvPath -Pattern '^PDP_TRIAL_ADMIN_PASSWORD=(.*)
$loginPath = Join-Path $ProjectRoot "PDP-ONE-LOCAL-LOGIN.txt"
$loginText = "PDP One local address: http://localhost:8080`r`nUsername: $adminUser`r`nPassword: $adminPassword`r`n`r`nKeep this file private."
[IO.File]::WriteAllText($loginPath, $loginText, [Text.UTF8Encoding]::new($false))
