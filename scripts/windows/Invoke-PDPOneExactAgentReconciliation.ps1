...
function Assert-ExactSteadyStateManifest($Contract) {
    if ([string]$Contract.schema -ne "pdp-one.deployment-agent-compatibility.v1") { throw "Candidate Agent compatibility contract schema is unsupported." }
    $protocol = [int]$Contract.protocol_version
    if ($protocol -ne $SupportedProtocolVersion) { throw "Candidate Agent protocol is unsupported." }
    $migrationProperty = $Contract.PSObject.Properties["migration_stage"]
    if ($null -ne $migrationProperty -and -not [string]::IsNullOrWhiteSpace([string]$migrationProperty.Value)) { throw "Automatic Agent reconciliation is disabled for migration-stage compatibility manifests." }
    $entries = @($Contract.bootstrap_files)
    if ($entries.Count -ne $ExpectedAgentFiles.Count) { throw "Steady-state Agent compatibility manifest has an unexpected bootstrap file count." }
    $seenFiles = @{}
    $seenPaths = @{}
    foreach ($entry in $entries) {
        ...
        if ($seenFiles.ContainsKey($agentFile) -or $seenPaths.ContainsKey($repositoryPath)) { throw "Candidate Agent manifest contains duplicate bootstrap entries." }
    ...
    if (($actual -join "`n") -ne ($expected -join "`n")) { throw "Candidate Agent manifest does not match the fixed steady-state bootstrap file set." }
    return $entries
}
...
try {
    ...
    if ([string]$commitResponse.sha -ne $CommitSha) { throw "GitHub did not return the exact requested Agent reconciliation commit." }
    ...
    foreach ($entry in $entries) {
        ...
        if ($installedExists) { Copy-Item -LiteralPath $installedPath -Destination $backupPath -Force }
    ...
    foreach ($record in $records) {
        ...
        if (-not [string]::Equals($stageHashNow, [string]$record.candidate_sha256, [StringComparison]::OrdinalIgnoreCase)) { throw "Candidate Agent bootstrap hash changed after download: $($record.file)" }
        ...
        if (-not [string]::Equals($installedHashAfter, [string]$record.candidate_sha256, [StringComparison]::OrdinalIgnoreCase)) { throw "Installed Agent bootstrap hash mismatch: $($record.file)" }
    }
    ...
    foreach ($record in $rollbackRecords) {
        try {
            if ([bool]$record.installed_before) {
                ...
                if (-not [string]::Equals($restoredHash, [string]$record.installed_sha256_before, [StringComparison]::OrdinalIgnoreCase)) { $rollbackSucceeded = $false }
            } else {
