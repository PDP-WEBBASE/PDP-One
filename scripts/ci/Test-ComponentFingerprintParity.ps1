...
try {
    ...
    foreach ($component in @('backend', 'mcp', 'web')) {
        ...
        Assert-PDPOneEqual $powershellFingerprint $zipballNodeFingerprint "$component Node-vs-PowerShell deployment zipball"
        if (-not [string]::Equals($checkoutFingerprint, $zipballNodeFingerprint, [System.StringComparison]::Ordinal)) {
            Write-Host ("checkout_representation_differs {0}=true; deployment zipball remains canonical" -f $component) -ForegroundColor Yellow
        }
    }

    Write-Host 'Exact GitHub deployment zipball Node and Windows PowerShell 5.1 fingerprints are equivalent.' -ForegroundColor Green
} finally {
