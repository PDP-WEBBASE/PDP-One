...
- Useful goods and service groups are included when available; transport-only source hashes are omitted from the model payload.
- Claim updates use a single bulk database write instead of one write per record.
- Import preloads Run Items, notice hashes and prior drafts, then bulk-updates compact results and notice statuses.
