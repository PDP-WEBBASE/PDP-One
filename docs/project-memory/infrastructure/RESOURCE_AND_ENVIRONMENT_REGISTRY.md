...
This registry preserves both current locators and historical paths. A recorded path does not prove current existence; verify live state before use.
...
| RES-DEP-02 | Deployment reports | exact deployment evidence | historically under `C:\ProgramData\PDP-One\deployment-agent\reports\...` | Deployment Agent | live locator pattern; do not assume a specific report without request ID |
...
| RES-WSL-01 | WSL/VHDX | Rancher/Docker backing environment | Windows | Windows/runtime | protected; no unregister/delete |
...
| RES-TAIL-01 | Tailscale/Funnel | remote/public connectivity | Tailscale identity/endpoints; secret/private details not stored | runtime | verify live before recovery |
...
| RES-BKP-03 | Historical portable backup path | external portable copies | historical E:/other storage mentioned in transfer lineage | user storage | historical locator only |
